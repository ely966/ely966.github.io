...
export class EditadminComponent implements OnInit {
  ...
  constructor(private router:Router, private formB: FormBuilder, private adminServi: AdministradorserviService) { }
...
/*=================================================================================== */
  ...
  editarAdmin(){
    if(this.editando == true){
      ...
      this.adminEditado = this.miFormulario.value;

      this.carga();

      this.adminServi.editardatosadmin(this.adminEditado).subscribe(
        {
          next: (
            datos => {
              Swal.fire({ //Notificación de que se ha editado correctamente
                title: 'Editado correctamente',
                text: "Tu usuario ha sido editada.",
                icon: 'success',
                position: 'center',
                timer: 2500
                })
